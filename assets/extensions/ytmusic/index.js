...
function cacheGet(k) {
  ...
  // Refresh insertion order so eviction follows true LRU semantics.
  _cache.delete(k);
  _cache.set(k, e);
  return e.v;
}
...
function poTokenCacheSet(k, token, expiresAt) {
  ...
  if (_poTokenCache.has(k)) _poTokenCache.delete(k);
  _poTokenCache.set(k, { token: token, expiresAt: safeExpiresAt });
  while (_poTokenCache.size > CONFIG.poTokenCacheMaxEntries) {
    var oldestKey = _poTokenCache.keys().next().value;
    _poTokenCache.delete(oldestKey);
  }
}
...
// Resolve and download one InnerTube candidate at a time. Fetching all client
// candidates up front adds three unnecessary player requests when the first
// client succeeds, and also lets otherwise fresh media URLs age before use.
function tryInnerTubeDownloadsLazily(videoID, pageInfo, outputPath, youtubeURL, coverURL) {
  for (var ci = 0; ci < INNERTUBE_CLIENTS.length; ci++) {
    var client = INNERTUBE_CLIENTS[ci];
    L("info", "[InnerTube] Resolving candidate from " + client.name + " for " + videoID);

    var candidate = _tryInnerTubeClient(videoID, client, pageInfo);
    if (!candidate || candidate.error) {
      L("warn", "[InnerTube] " + client.name + " failed:", candidate && candidate.error ? candidate.error : "no candidate");
      continue;
    }

    L("info", "[YTMusic] Trying direct InnerTube download:", candidate.clientName, "itag=" + candidate.itag, candidate.extension, "pot=" + (candidate.poTokenUsed ? "yes" : "no"));
    var directAttempt = attemptDirectCandidateDownload(candidate, outputPath, youtubeURL);
    var directResult = directAttempt.result;
    if (directResult && directResult.success) {
      L("info", "[YTMusic] Direct InnerTube download OK:", candidate.clientName, "itag=" + candidate.itag);
      return buildDownloadSuccess(directResult.path || directAttempt.path, coverURL);
    }

    L("warn", "[YTMusic] Direct InnerTube download failed:", candidate.clientName, downloadErrorText(directResult));
    if (candidate.needsGvsPoToken && isDownloadAuthFailure(directResult)) {
      var refreshedCandidate = refreshInnerTubeAudioCandidate(videoID, candidate, pageInfo);
      if (refreshedCandidate && refreshedCandidate.url && refreshedCandidate.url !== candidate.url) {
        ...
        if (retryResult && retryResult.success) {
          ...
          return buildDownloadSuccess(retryResult.path || retryAttempt.path, coverURL);
        }
...
function runsText(runs, separator) {
  ...
  return runs.map(function(run) { return run && run.text ? String(run.text) : ""; })
...
function findFirstRenderer(node, rendererKey, depth) {
  ...
  if (Object.prototype.hasOwnProperty.call(node, rendererKey) && node[rendererKey]) {
  ...
  for (var key in node) {
    if (!Object.prototype.hasOwnProperty.call(node, key) || key === "trackingParams" || key === "clickTrackingParams") continue;
...
function browsePageType(endpoint) {
  try {
    return String(endpoint.browseEndpointContextSupportedConfigs
...
function collectTrackReferences(node) {
  ...
  function visit(value, depth) {
    ...
    if (value.browseEndpoint && value.browseEndpoint.browseId) {
      ...
      if ((browseId.startsWith("UC") || pageType === "MUSIC_PAGE_TYPE_ARTIST") && text) {
        if (!seenArtists[browseId]) {
          seenArtists[browseId] = true;
          result.artists.push({ id: browseId, name: text });
        }
      } else if ((browseId.startsWith("MPREb_") || pageType === "MUSIC_PAGE_TYPE_ALBUM") && !result.album_id) {
        result.album_id = browseId;
        result.album_name = text;
      } else if ((browseId.startsWith("MPTC") || pageType === "MUSIC_PAGE_TYPE_TRACK_CREDITS") && !result.credits_id) {
    ...
    for (var key in value) {
      ...
      if (key === "navigationEndpoint" && child && typeof child === "object") {
        ...
        if (typeof value.text === "string") decorated.__runText = value.text.trim();
...
function hasExplicitBadge(node) {
  try {
    ...
    for (var i = 0; i < badges.length; i++) {
      ...
      var iconType = inline.icon && inline.icon.iconType || metadata.icon && metadata.icon.iconType || "";
      var label = inline.accessibilityData && inline.accessibilityData.accessibilityData && inline.accessibilityData.accessibilityData.label || metadata.label || "";
      if (String(iconType).toUpperCase().indexOf("EXPLICIT") >= 0 || String(label).toLowerCase() === "explicit") {
        return true;
      }
    }
  } catch (e) {}
...
function parseItemExtended(info) {
  try {
          ...
          for (var ri = 0; ri < fcr2.text.runs.length; ri++) {
            ...
            if (run && run.text) {
              ...
              if (run.navigationEndpoint && run.navigationEndpoint.browseEndpoint) {
                ...
                if (String(artistBrowse.browseId || "").startsWith("UC") || browsePageType(artistBrowse) === "MUSIC_PAGE_TYPE_ARTIST") {
    ...
    if (!artist && c.subtitle && c.subtitle.runs) {
      ...
      for (var si = 0; si < c.subtitle.runs.length; si++) {
        ...
        if (srun && srun.text) {
          ...
          if (srun.navigationEndpoint && srun.navigationEndpoint.browseEndpoint) {
            var subtitleBrowse = srun.navigationEndpoint.browseEndpoint;
            if (String(subtitleBrowse.browseId || "").startsWith("UC") || browsePageType(subtitleBrowse) === "MUSIC_PAGE_TYPE_ARTIST") {
    ...
    if (references.artists.length > 0) {
      artist = uniqueStrings(references.artists.map(function(item) { return item.name; })).join(", ");
    }
    ...
    return {
      ...
      artist_url: references.artists.length ? "https://music.youtube.com/channel/" + references.artists[0].id : "",
      album_id: references.album_id,
      album_url: references.album_id ? "https://music.youtube.com/browse/" + references.album_id : "",
      external_urls: "https://music.youtube.com/watch?v=" + String(videoId),
      external_links: { youtube: "https://music.youtube.com/watch?v=" + String(videoId) },
...
function sanitizeTrackBeforeReturn(t) {
  ...
  var sanitized = {
    ...
    external_urls: String(t.external_urls || "https://music.youtube.com/watch?v=" + id).trim(),
    external_links: Object.assign({ youtube: "https://music.youtube.com/watch?v=" + id }, t.external_links || {}),
...
function applyBrowseCollectionMetadata(parsed, collection, isPlaylist) {
  ...
  parsed.album_url = parsed.album_id ? "https://music.youtube.com/browse/" + parsed.album_id : "";
  ...
  if (!parsed.thumbnail && collection.cover_url) parsed.thumbnail = collection.cover_url;
  ...
  if (!parsed.artist_id && collection.artist_id) {
    ...
    parsed.artist_url = "https://music.youtube.com/channel/" + collection.artist_id;
  }
...
function applyResponsiveHeaderMetadata(renderer, info) {
  ...
  for (var i = 0; i < subtitleRuns.length; i++) {
    var subtitleText = String(subtitleRuns[i] && subtitleRuns[i].text || "").trim();
    var lower = subtitleText.toLowerCase();
    if (lower === "album" || lower === "single" || lower === "ep" || lower === "playlist") {
  ...
  var artistRuns = renderer.straplineTextOne && renderer.straplineTextOne.runs || [];
  if (artistRuns.length) {
    info.artists = uniqueStrings(artistRuns.map(function(run) { return run && run.text; })).join(", ");
    for (var ar = 0; ar < artistRuns.length; ar++) {
      var endpoint = artistRuns[ar] && artistRuns[ar].navigationEndpoint && artistRuns[ar].navigationEndpoint.browseEndpoint;
      if (endpoint && (String(endpoint.browseId || "").startsWith("UC") || browsePageType(endpoint) === "MUSIC_PAGE_TYPE_ARTIST")) {
  ...
  if (renderer.thumbnail && renderer.thumbnail.musicThumbnailRenderer && renderer.thumbnail.musicThumbnailRenderer.thumbnail) {
    var thumbnailURL = pickLastThumbnailUrl(renderer.thumbnail.musicThumbnailRenderer.thumbnail.thumbnails);
    if (thumbnailURL) info.cover_url = makeSquareThumb(thumbnailURL);
  }

  var secondSubtitle = runsText(renderer.secondSubtitle && renderer.secondSubtitle.runs || []);
  var totalMatch = secondSubtitle.match(/(\d[\d,.]*)\s+(songs?|tracks?)/i);
  if (totalMatch) {
    info.total_tracks = parseInt(totalMatch[1].replace(/[^\d]/g, ""), 10) || info.total_tracks || 0;
  }
  return info;
}
...
function _fetchAlbumHeaderMetadata(albumBrowseId) {
  try {
    ...
    var info = { name: "", artists: "", artist_id: "", cover_url: null, release_date: "", album_type: "album", total_tracks: 0 };

    var responsiveHeader = data.header && data.header.musicResponsiveHeaderRenderer ||
  ...
  while (continuationToken && page < MAX_CONTINUATION_PAGES) {
    ...
    for (var i = 0; i < contResult.tracks.length; i++) {
      ...
      if (parsed) {
        ...
        applyBrowseCollectionMetadata(parsed, result.album, result.type === "playlist");
  ...
  if (!result.name && result.tracks && result.tracks.length > 0) {
    ...
    if (albumBrowseId) {
      ...
      if (albumMeta) {
        ...
        result.album.total_tracks = albumMeta.total_tracks || result.album.total_tracks;
...
var headerInfo = {
    ...
    if (!header && data.contents) {
      var nestedResponsiveHeader = findFirstRenderer(data.contents, "musicResponsiveHeaderRenderer");
      if (nestedResponsiveHeader) header = { musicResponsiveHeaderRenderer: nestedResponsiveHeader };
    }
    ...
    if (header) {
      if (header.musicResponsiveHeaderRenderer) {
        applyResponsiveHeaderMetadata(header.musicResponsiveHeaderRenderer, headerInfo);
        L("debug", "musicResponsiveHeaderRenderer cover_url", headerInfo.cover_url);
      }
if (header.musicDetailHeaderRenderer) {
    ...
    headerInfo.total_tracks = Math.max(headerInfo.total_tracks || 0, tracks.length);
    for (var trackIndex = 0; trackIndex < tracks.length; trackIndex++) {
      tracks[trackIndex].total_tracks = headerInfo.total_tracks;
      if (!tracks[trackIndex].album_artist) tracks[trackIndex].album_artist = headerInfo.artists;
      if (!tracks[trackIndex].release_date) tracks[trackIndex].release_date = headerInfo.release_date;
      if (!tracks[trackIndex].album_type) tracks[trackIndex].album_type = headerInfo.album_type;
    }
...
function fetchVideoMetadataSync(videoId) {
  var data = fetchJSONSync("https://music.youtube.com/youtubei/v1/player?alt=json", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "User-Agent": getRandomUserAgent(),
  ...
  if (!data || !data.videoDetails) return null;
  var details = data.videoDetails;
  var thumbnailURL = pickLastThumbnailUrl(details.thumbnail && details.thumbnail.thumbnails);
  return {
    id: videoId,
...
function getTrack(trackID) {
  ...
  if (!raw) return null;
  var candidates = performSearchSync((raw.artist ? raw.artist + " " : "") + raw.title, YT_SEARCH_PARAMS.tracks);
  for (var i = 0; i < candidates.length; i++) {
...
function fetchJSONSync(url, options) {
  try {
    var response = fetch(url, options || { method: "GET", headers: { "User-Agent": getRandomUserAgent() } });
...
function primaryArtistMatches(expected, actual) {
  var expectedPrimary = String(expected || "").split(/\s*,\s*|\s+feat\.?\s+|\s+ft\.?\s+/i)[0];
  return metadataMatchValue(expectedPrimary) === metadataMatchValue(actual);
}

function parseTrackCredits(data) {
  var result = { title: "", artists: "", composer: "", producer: "", release_date: "", cover_url: null };
  var actions = data && data.onResponseReceivedActions;
  if (!Array.isArray(actions)) return result;
  for (var ai = 0; ai < actions.length; ai++) {
    var dialog = actions[ai] && actions[ai].openPopupAction && actions[ai].openPopupAction.popup &&
      actions[ai].openPopupAction.popup.dismissableDialogRenderer;
    if (!dialog) continue;
    var metadata = dialog.metadata && dialog.metadata.musicMultiRowListItemRenderer;
    if (metadata) {
      result.title = runsText(metadata.title && metadata.title.runs || []);
      result.artists = runsText(metadata.subtitle && metadata.subtitle.runs || []);
      var secondTitle = runsText(metadata.secondTitle && metadata.secondTitle.runs || []);
      var yearMatch = secondTitle.match(/\b(19|20)\d{2}\b/);
      if (yearMatch) result.release_date = yearMatch[0];
      var thumbnailURL = pickLastThumbnailUrl(metadata.thumbnail && metadata.thumbnail.musicThumbnailRenderer &&
        metadata.thumbnail.musicThumbnailRenderer.thumbnail && metadata.thumbnail.musicThumbnailRenderer.thumbnail.thumbnails);
      if (thumbnailURL) result.cover_url = makeSquareThumb(thumbnailURL);
    }
    var sections = dialog.sections || [];
    for (var si = 0; si < sections.length; si++) {
      var section = sections[si] && sections[si].dismissableDialogContentSectionRenderer;
      if (!section) continue;
      var role = runsText(section.title && section.title.runs || []).toLowerCase();
      var names = uniqueStrings((section.subtitle && section.subtitle.runs || []).map(function(run) {
...
function fetchTrackCreditsSync(videoID) {
  ...
  var cached = cacheGet(cacheKey);
  if (cached) return cached;
  var data = fetchJSONSync("https://music.youtube.com/youtubei/v1/browse?alt=json", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
function deezerComposerNames(trackData) {
  ...
  for (var i = 0; i < contributors.length; i++) {
    var role = String(contributors[i] && contributors[i].role || "").toLowerCase();
    if (/composer|songwriter|writer|author/.test(role)) names.push(contributors[i].name);
  }
...
function fetchValidatedDeezerMetadata(track) {
  ...
  var durationSeconds = Math.round(Number(track.duration_ms || 0) / 1000) || Number(track.duration || 0) || 0;
  if (!title || !artists) return null;

  var query = 'track:"' + title.replace(/"/g, "") + '" artist:"' + artists.split(",")[0].replace(/"/g, "") + '"';
  if (albumName) query += ' album:"' + albumName.replace(/"/g, "") + '"';
  var search = fetchJSONSync("https://api.deezer.com/search?q=" + encodeURIComponent(query) + "&limit=10");
  ...
  for (var i = 0; i < search.data.length; i++) {
    ...
    if (!primaryArtistMatches(artists, item.artist && item.artist.name)) continue;
    if (albumName && metadataMatchValue(item.album && item.album.title) !== metadataMatchValue(albumName)) continue;
    if (durationSeconds && item.duration && Math.abs(Number(item.duration) - durationSeconds) > 5) continue;
    candidate = item;
    break;
  }
  if (!candidate) return null;

  var trackData = fetchJSONSync("https://api.deezer.com/track/" + encodeURIComponent(String(candidate.id)));
  if (!trackData || metadataMatchValue(trackData.title) !== metadataMatchValue(title) ||
      !primaryArtistMatches(artists, trackData.artist && trackData.artist.name)) return null;
  if (durationSeconds && trackData.duration && Math.abs(Number(trackData.duration) - durationSeconds) > 5) return null;

  var deezerAlbumName = String(trackData.album && trackData.album.title || candidate.album && candidate.album.title || "");
  var sameAlbum = !!albumName && metadataMatchValue(deezerAlbumName) === metadataMatchValue(albumName);
  var albumData = sameAlbum && trackData.album && trackData.album.id
    ? fetchJSONSync("https://api.deezer.com/album/" + encodeURIComponent(String(trackData.album.id)))
    : null;
  if (albumData && metadataMatchValue(albumData.title) !== metadataMatchValue(albumName)) albumData = null;

  var deezerURL = String(trackData.link || "https://www.deezer.com/track/" + candidate.id);
  var result = {
  ...
  if (albumData) {
    result.album_artist = String(albumData.artist && albumData.artist.name || "").trim();
    ...
    if (albumData.tracks && Array.isArray(albumData.tracks.data)) {
      for (var ti = 0; ti < albumData.tracks.data.length; ti++) {
        result.total_discs = Math.max(result.total_discs, Number(albumData.tracks.data[ti].disk_number || 1));
      }
    }
    ...
    result.genre = uniqueStrings(genres.map(function(genre) { return genre && genre.name; })).join("; ");
  }
  return result;
}
...
function enrichTrack(track) {
  ...
  var youtubeURL = "https://music.youtube.com/watch?v=" + encodeURIComponent(track.id);
  var enrichment = {
    external_urls: youtubeURL,
    external_links: Object.assign({}, track.external_links || {}, { youtube: youtubeURL })
  };
  ...
  if (credits) {
    ...
    if (!track.release_date && credits.release_date) enrichment.release_date = credits.release_date;
    if (!track.cover_url && credits.cover_url) enrichment.cover_url = credits.cover_url;
  }
  ...
  if (albumID.startsWith("MPREb_")) {
    ...
    if (nativeAlbum) {
      if (!track.album_name && nativeAlbum.name) enrichment.album_name = nativeAlbum.name;
      if (!track.album_artist && nativeAlbum.artists) enrichment.album_artist = nativeAlbum.artists;
      if (!track.artist_id && nativeAlbum.artist_id) enrichment.artist_id = nativeAlbum.artist_id;
      if (!track.artist_url && nativeAlbum.artist_id) enrichment.artist_url = "https://music.youtube.com/channel/" + nativeAlbum.artist_id;
      if (!track.release_date && nativeAlbum.release_date) enrichment.release_date = nativeAlbum.release_date;
      if (!track.album_type && nativeAlbum.album_type) enrichment.album_type = nativeAlbum.album_type;
      if (!track.total_tracks && nativeAlbum.total_tracks) enrichment.total_tracks = nativeAlbum.total_tracks;
      if (!track.cover_url && nativeAlbum.cover_url) enrichment.cover_url = nativeAlbum.cover_url;
    }
  }
  ...
  if (deezer) {
    ...
    if (!enrichment.composer && !track.composer && deezer.composer) enrichment.composer = deezer.composer;
    ...
    var albumFields = ["album_artist", "track_number", "total_tracks", "disc_number", "total_discs", "album_type", "upc", "label", "copyright", "genre"];
    for (var fi = 0; fi < albumFields.length; fi++) {
      var field = albumFields[fi];
      if ((!track[field] || field === "genre" || field === "label" || field === "copyright" || field === "upc") && deezer[field]) {
        enrichment[field] = deezer[field];
      }
    }
    if (deezer.release_date && (!track.release_date || /^\d{4}$/.test(String(track.release_date)))) {
      enrichment.release_date = deezer.release_date;
    }
  }

  cacheSet(cacheKey, enrichment);
  L("info", "enrichTrack: success", { id: track.id, hasComposer: !!enrichment.composer, hasIsrc: !!enrichment.isrc, hasGenre: !!enrichment.genre });
...
registerExtension({
  ...
  download: function(trackID, quality, outputPath, onProgress) {
    ...
    var directDownloadResult = null;
    try {
      directDownloadResult = tryInnerTubeDownloadsLazily(
        videoID,
        pageInfo,
        outputPath,
        youtubeURL,
      ...
      L("warn", "[YTMusic] Lazy InnerTube download failed:", String(innerTubeError));
    }
    if (directDownloadResult && directDownloadResult.success) return directDownloadResult;
