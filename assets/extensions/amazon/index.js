...
// v2.3.3 - Reuses host-prepared track context to avoid duplicate resolution.
...
function fetchWithRetry(requestFn) {
  ...
  for (var attempt = 0; attempt < CONFIG.maxAttempts; attempt++) {
    if (utils && typeof utils.isDownloadCancelled === "function" && utils.isDownloadCancelled()) {
      L("info", "[Amazon] Retry cancelled before attempt " + (attempt + 1));
      return null;
    }
    if (attempt > 0) {
      var jitteredDelay = delay + Math.floor(Math.random() * Math.max(25, Math.floor(delay / 4)));
      L("info", "[Amazon] Retry " + (attempt + 1) + "/" + CONFIG.maxAttempts + " after " + jitteredDelay + "ms");
      if (!sleep(jitteredDelay)) return null;
...
registerExtension({
  initialize: function() {
    L("info", "[Amazon] Extension v2.3.3 init");
    initSession();
    return true;
  },
